...
namespace swing {
	...
	namespace tensor {
		...
		/*
		template<typename T>
		Tensor<T>::Tensor(swing::RefArray<T> arg) :
			size(arg.size()),
			iterable(new T[arg.size()]),
			shape(arg.size()) {
		...
		template<typename T>
		void Tensor<T>::permute(size_t* permutations) {
			...
			reorder(new_shape.shape, this->shape.shape, permutations, this->shape.dims);
			...
			for (size_t index = 0; index < this->size; index++) {
				...
				// reorder indicies
				reorder(new_indicies, indicies, permutations, this->shape.dims);
